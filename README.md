...
本人另有一個純前端版本的股票分析工具 [tw-stock-analyzer](https://github.com/caryhp2-cell/tw-stock-analyzer)，兩者架構不同：

| | **stock-chart-analysis（本專案）** | **tw-stock-analyzer** |
|--|--|--|
| 框架 | Streamlit（Python）| 純 HTML + JS |
| 運作位置 | 伺服器端（Server-side） | 瀏覽器端（Client-side）|
| 資料來源 | Yahoo Finance（yfinance）| TWSE / TPEx / FinMind |
| 需要 Python server | 是 | 否 |
| 部署方式 | 本機執行 | Vercel（靜態 CDN）|
| 類比 | SSR（Next.js Server-side） | SPA（純前端）|

> 兩者都是「一個 URL 進去全搞定」，但運作原理不同。本專案需要 Python server 持續運行，所有邏輯（資料抓取、計算、畫圖）都在 server 端執行，結果再傳回瀏覽器；tw-stock-analyzer 則是純靜態，直接部署到 Vercel CDN，瀏覽器自己打 API 拿資料。
